...
def _read_categorical_column(col_group: h5py.Group, start: int, end: int, cache: Dict[int, np.ndarray]) -> List[str]:
    """
...
def table(
    ...
    columns: Optional[str] = typer.Option(None, "--cols", "-c", help="Columns to include in the output table"),
    out: Optional[Path] = typer.Option(None, "--out", "-o", help="Output file path (defaults to stdout)"),
    chunk_rows: int = typer.Option(10000, "--chunk-rows", "-r", help="Number of rows to read per chunk"),
) -> None:
    """
    ...
    with open_file(file) as f:
        ...
        if col_list:
            ...
            if col_order is not None:
                ...
                col_names = sorted(
                    name for name in group.keys()
                    if isinstance(group[name], (h5py.Dataset, h5py.Group)) and name != index_name
                )
